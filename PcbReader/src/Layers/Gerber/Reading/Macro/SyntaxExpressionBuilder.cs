        ...
        for (var i = index; i < _tokens!.Count; i++) {
            switch (_tokens[i]) {
                ...
                case ParensCloseToken:
                    if(index==0) throw new ApplicationException("Unexpected parent close");
                    return (result, shift);
                case OperationToken ot:
                    if (isOdd) throw new ApplicationException("Unexpected operator, must be operator");
                    var sop = new SyntaxOperator(ot.Type);
                    result.Parts.Add(sop);
                    break;
                case ValueToken vt:
                    if (!isOdd) throw new ApplicationException("Unexpected operand, must be operator");
