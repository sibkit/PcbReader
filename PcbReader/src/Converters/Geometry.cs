...
public static class Geometry {
    ...
    public static Point ArcCenter(Point sp, Point ep, double radius, RotationDirection rd) {
        ...
        return rd == RotationDirection.ClockWise ? 
            new Point(p0.X + (ep.Y - sp.Y) * h / d, p0.Y - (ep.X - sp.X) * h / d) : 
            new Point(p0.X - (ep.X - sp.Y) * h / d, p0.Y - (ep.X - sp.X) * h / d);
    }
    
    public static double ArcAngel(Point sp, Point ep, Point cp, RotationDirection rd) {
